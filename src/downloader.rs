...
impl DownloadRequest {
            ...
            for entry in current_dir().unwrap().read_dir().unwrap() {
                ...
                if entry.file_name().to_str().unwrap() == playlist.title {
                    for inner_entry in entry.path().read_dir().unwrap() {
                        ...
                        if inner_entry.file_type().unwrap().is_dir() { continue; }
    ...
    pub async fn download_playlist(self: &Self, cookies: Option<String>) {
        ...
        for (video_id, video) in &self.missing_videos {
            println!("Downloading: {} | {} out of {}", video.formatted_title, counter, self.missing_videos.len());
            // , "--cookies", cookies
            ...
            match Tag::read_from_path(&video.path) {
                ...
                Err(_) => {
                    if !video.path.exists() {
                        println!("Failed to download: {}", video_id);
                        println!("You should check if the video is age-restricted, if it is you need to pass cookies, to learn about that just do -h.");
                        println!("Otherwise please report it on my github!")
                    }
                    else {
                        println!("Failed to read tag from: {},", video_id);
                        println!("You should remove the file as it will have no tags and the program will try to download it again.");
                    }
                },
            };
            println!("Finished downloading: {} | {} out of {}", video.formatted_title, counter, self.missing_videos.len());
            counter+=1;
        }
    }
    ...
    pub async fn remove_vidoes(self: &Self) {
        for (_video_id, video) in &self.remove_vidoes {
            println!("This video IS NOT FUCKING REAL: {}, ITS FAKE LOCATION IS: {}", video.title, &video.path.display())
        }
    }
}
